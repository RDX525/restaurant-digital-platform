...
import { LoginAuthPanel } from "@/components/platform/LoginAuthPanel";
...
/** Server chrome around the auth panel so landing paints without waiting on auth JS. */
...
function AuthPanelBackground() {
  return (
    <>
      <div
        className="pointer-events-none absolute -right-24 top-0 h-72 w-72 rounded-full bg-gold-500/10 blur-2xl"
        aria-hidden="true"
      />
      <div
        className="pointer-events-none absolute -left-16 bottom-20 h-56 w-56 rounded-full bg-pine-500/8 blur-2xl"
        aria-hidden="true"
      />
    </>
  );
